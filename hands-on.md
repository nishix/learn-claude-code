...
Claude Code では、`/` で始まるコマンドで様々な機能を素早く実行できます。
ハンズオン中に活躍する主要コマンドを紹介します！
...
/help
```
Claude Code の全機能や使い方を確認できます。困ったときはまずこれ！
...
**例：**
```
/refactor src/App.jsx - 冗長なコードを整理
```
コードの品質を改善します。
...
/commit
```
Git の変更内容から自動で適切なコミットメッセージを生成します。
...
**✏️ ファイルを修正するとき：**
```
Claude Code に「/find App.css で display: none を探して」と指示
↓
ファイルと行番号が表示される
↓
手動でファイルを開いて修正
...
✅ **日本語での指示で OK**
```
「ファイルを探してください」という日本語指示で動作します
```

✅ **複数のコマンドを組み合わせる**
```
/search で検索 → /refactor で修正 → /test で確認
```

✅ **曖昧な指示でも理解してくれる**
```
「Todoの状態を管理している部分を見つけて」でも OK
```
